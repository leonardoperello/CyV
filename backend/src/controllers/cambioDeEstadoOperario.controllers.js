    ...
    if (tareasFiltradas.length) {
      ...
      const otiFiltrada = otis.find((oti) =>
        oti.tareas.find((tarea) =>
          tareasFiltradas.find((tareaOp) => tareaOp._id.equals(tarea._id))
        )
      );
...
export async function cambioEstadotareaAsignada(data) {
  ...
  if (oti && tarea) {
    if (
      data.tipoEstado.nombre === "detenida" ||
      data.tipoEstado.nombre === "abortada"
    ) {
      //quiero pasar al estado detenida
      ...
      if (
        ...
        tarea.estado[tarea.estado.length - 1].fechaFin =
          moment().format("YYYY/MM/DD");
        tarea.estado.push(estado); //actualizo el esta de la tarea a detenida o abortada
        oti.estados[oti.estados.length - 1].fechaFin =
          moment().format("YYYY/MM/DD");
        oti.estados.push(estado); // actualiza el estado de la oti a detenida o abortada

        oti.tareas.find((tareaOti) => {
          // busco en la oti la tarea y actualizo el estato
          if (tareaOti._id.equals(tarea._id)) {
            tareaOti.estado[tareaOti.estado.length - 1].fechaFin =
              moment().format("YYYY/MM/DD");
    ...
    if (data.tipoEstado.nombre === "finalizada") {
      //quiero finalizar la tarea .No controlo el estado general de oti xq si estoy en progreso el estado de la oti sigue en progreso

      if (
        tarea.estado[tarea.estado.length - 1]?.tipoEstado?.nombre ===
        "en progreso"
      ) {
        // controlo que el estado actual sea en progreso
        const estado = await cargarEstado(data);
        ...
        if (ultimatarea._id.equals(tarea._id)) {
          //controlo que sea la ultima tarea , si no es la utlima oti sigue ne estado en progreso
          ...
          if (sector) {
            const TareasSector = oti.tareas.filter(
              (tarea) => tarea.sector.nombre === sector.nombre
            ); //aca me quedo con las tareas de mi sector
            ...
            if (tareasFiltradas.length > 0) {
              //quedan tareas por hacer   ----no importa el orden en este caso xq eso se controla en la asignacion de tarea al operario
              ...
              const indice = oti.sector.findIndex(
                (sectorOti) => sectorOti.nombre === sector.nombre
              );
    ...
    if (data.tipoEstado.nombre === "en progreso") {
      if (
        ...
        // contorlo que el estado anterior sea en progreso si no dejo al estado detenida
        ...
        if (
          ...
          //si ya esta en progreso no lo cambio
